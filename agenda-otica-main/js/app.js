...
const DB = {
  ...
  async refresh() {
    const [storesRes, appointmentsRes, clientsRes] = await Promise.all([
...
const App = {
  ...
  updateConnStatus() {
    ...
    el.className = `conn-status ${DB.connected ? 'online' : 'offline'}`;
    el.innerHTML = `<span class="pulse"></span>${DB.connected ? 'Online' : 'Offline'}`;
  },
